...
for style_folder in file_list:
    ...
    for file_name in os.listdir(now_style_folder_path):
        ...
        data = json.load(file)
        print(data["데이터셋 정보"]["데이터셋 상세설명"]["라벨링"]["상의"][0]["카테고리"])
        print(data["데이터셋 정보"]["데이터셋 상세설명"]["라벨링"]["하의"][0]["카테고리"])
        print(data["데이터셋 정보"]["데이터셋 상세설명"]["라벨링"]["아우터"][0]["카테고리"])
        print(data["데이터셋 정보"]["데이터셋 상세설명"]["라벨링"]["원피스"][0]["카테고리"])
        exit(0)
        count = 0
        check = [False, False, False, False]
