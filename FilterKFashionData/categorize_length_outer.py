    ...
    for file_name in os.listdir(now_style_folder_path):
        ...
        if count == 1:
            ...
            if data["데이터셋 정보"]["데이터셋 상세설명"]["라벨링"][now_category_main][0].get("기장") != None:
                ...
                # json 파일 복사
                shutil.copyfile(now_style_folder_path + file_name, base_dir + "5length_outer/" + now_fit + "/label/" + file_name)
                # 이미지 파일 복사
                shutil.copyfile(base_dir + style_folder + "/" + file_name_num + ".jpg", base_dir + "5length_outer/" + now_fit + "/image/" + file_name_num + ".jpg")
